...
public class OASSolver extends Solver {
    ...
    private List<Constraint> mapVariables(Map<String, List<String>> data, List<Parameter> parameters) throws IDLException {
        ...
        for (Parameter parameter : parameters) {
            ...
            boolean paramIsString = paramType.equals(ParameterType.STRING.toString()) || paramType.equals(ParameterType.ARRAY.toString());
            boolean paramIsNumber = paramType.equals(ParameterType.INTEGER.toString()) || paramType.equals(ParameterType.NUMBER.toString());
            ...
            if (data == null) { // When initializing the solver (without data)
                ...
                if (paramData != null && !paramData.isEmpty()) {
                    if (paramType.equals(ParameterType.BOOLEAN.toString())) {
                        ...
                        if (domain.length > 0)
                            this.getVariable(Utils.parseIDLParamName(parameter.getName()), IntVar.class, true, getBooleanDomain(paramData));
                        else
            ...
            if (disabledParameter) {
                ...
                varParamSet = this.getVariable(Utils.parseIDLParamName(parameter.getName()) + "Set", IntVar.class, true, 0).asIntVar();
            } else {
                varParamSet = this.getVariable(Utils.parseIDLParamName(parameter.getName()) + "Set", IntVar.class, true, 0, 1).asIntVar();
            }
